...
- nombre del cluster: `farmacia-bi-debezium`
...
```powershell
docker exec farmacia-bi-debezium-kafka-1 bash -lc "kafka-topics.sh --bootstrap-server kafka:9092 --list"
```

Ver eventos de una tabla, por ejemplo `familias`:

```powershell
docker exec farmacia-bi-debezium-kafka-1 bash -lc "kafka-console-consumer.sh --bootstrap-server kafka:9092 --topic farmadb.farmadb.familias --from-beginning"
```
